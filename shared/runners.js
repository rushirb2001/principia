...
//
// THE ONLY COPY. extension/src/runners.js and cli/src/runners.js are one-line
// re-exports of this file. They used to be byte-identical 165-line copies kept
// in sync by hand, and that is exactly how four wrong invocations shipped:
// every one had to be fixed twice, and nothing would have complained if only
// one had been. `node scripts/check-runners.js` fails if the runner ids here
// disagree with the contract, the validator or the MCP server, which are
// deliberately dependency-free and therefore still spell the list out.
//
// Note the CLI only ever needed detect(); the command builders below are the
// extension's concern. Reaching for a copy was never necessary.
